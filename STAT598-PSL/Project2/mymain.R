...
seasonal_naive_model<- function(train_ts, test_ts){
  num_forecasts <- nrow(test_ts)
  train_ts[is.na(train_ts)] <- 0
...
stlf_model<- function(train_ts, test_ts){
  ...
  #stlf requires 2 full seasons (52*2 = 104); so until we hit the 2 seasons
  # we use a linear forecast model
  #REF: https://piazza.com/class/jky28ddlhmu2r8?cid=393
  if(nrow(train_ts) < 104)
  {
    # naive forecast per store
    for(j in 2:ncol(train_ts)){
      store_ts <- ts(train_ts[, j], frequency=52)
      m <- tslm(store_ts ~ trend + season)               #fit linear model
      test_ts[, j] <- forecast(m, h=num_forecasts)$mean  #use model to create forecast and store results
    }
  }else{
    # naive forecast per store
    for(j in 2:ncol(train_ts)){
      store_ts <- ts(train_ts[, j], frequency=52)
      test_ts[, j] <- stlf(store_ts, h=num_forecasts, method="arima")$mean
    }  
  }
  test_ts
}
...
mypredict <- function() {
    ...
    for (dept in test_depts) {
        ...
        #mytemp <<- c(mytemp, nrow(train_dept_ts)) #testing
        ...
        f_seasonal_naive <- seasonal_naive_model(train_dept_ts, test_dept_ts)
        test_month <- update_forecast(test_month, f_seasonal_naive, dept, 2)
